...
- [ ] 类型检查通过，工具注册名恰好是三个指定名字。
- [ ] readfile 返回磁盘实际内容，writefile 真正创建文件。
- [ ] run_command 返回实际 stdout、stderr 和退出码。
- [ ] Agent 自己写脚本并执行，生成统计正确的 report.md。
