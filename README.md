...
这是你亲手实现的练习项目，完整参考项目位于 `../ink-agent-course/`。当前阶段：**01，运行第一个 TypeScript 程序**。
...
| 01 | 运行第一个 TypeScript 程序 | [开始本阶段](stages/01-typescript.md)，待执行 |
| 02 | 终端输入与输入循环 | 待进入时编写 |
...
所有命令默认在 `ink-agent-from-scratch/` 根目录执行。现在打开 [阶段 01](stages/01-typescript.md)，先提交说明，再开始实现。
