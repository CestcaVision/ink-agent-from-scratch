...
这是你亲手实现的练习项目，完整参考项目位于 `../ink-agent-course/`。当前阶段：**07，第一个工具与多步执行循环**。在 Ink 聊天中注册时间查询工具，观察模型调用、本地执行、结果续接与步数限制。
...
| 07 | 第一个工具与多步执行循环 | [开始本阶段](stages/07-tools-loop.md)，待执行 |
...
所有命令默认在 `ink-agent-from-scratch/` 根目录执行。现在打开 [阶段 07](stages/07-tools-loop.md)，先提交说明，再开始实现。
